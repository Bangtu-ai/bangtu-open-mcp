...
function createServer() {
    ...
    server.tool("bangtu_create_dwg_task", "上传 DWG 图纸并创建图纸基本信息识别任务。远程 Agent 优先传 fileBase64 和 fileName（平台附件转 Base64）；本地部署也支持 filePath 或 fileUrl，三种方式必须且只能选择一种。任务创建成功只表示后台任务已启动；请保存返回的 taskId，随后使用 bangtu_get_task_status 轮询至 SUCCESS，再调用 bangtu_get_frame_result 获取图框、图签和坐标等结构化解析结果。", {
        apiKey: apiKeySchema,
        ...
        if (sources !== 1)
            throw new Error("Provide exactly one of fileBase64, filePath, or fileUrl");
        if (fileBase64 && !fileName)
            throw new Error("fileName is required when fileBase64 is provided");
        if (!fileBase64 && fileName)
            throw new Error("fileName is only valid with fileBase64");
    ...
    server.tool("bangtu_create_cv_task", "根据已完成的 DWG 图框 frameId 创建建筑专业构件识别任务。必须传入 apiKey。任务创建后必须用 bangtu_get_task_status 轮询至 SUCCESS；不要根据创建接口返回成功就直接取结果。", {
        apiKey: apiKeySchema,
        ...
        const data = await callBangtuApi("/cv/building_cv/createTask", {
            method: "POST",
            form: { frameId },
            apiKey,
        });
    ...
    server.tool("bangtu_get_task_status", `查询帮图异步识别任务状态。任务为异步执行，返回 data.status：RUNNING 表示任务执行中，请等待约 ${Math.round(pollIntervalMs / 1000)} 秒后再次调用本工具；SUCCESS 表示任务完成，可继续调用对应结果获取工具；FAILED 表示任务失败，请查看 data.logs。DWG 图纸解析是耗时操作，复杂图纸最长可能需要约 ${maxTaskDurationMinutes} 分钟，请耐心轮询，不要因短时间未完成而判定失败。任务是否完成以 data.status 为准，不是本次查询的 code 字段。`, {
        apiKey: apiKeySchema,
    ...
    server.tool("bangtu_wait_task", `短时便捷轮询工具：本次调用最多持续 ${maxWaitSeconds} 秒，按 ${Math.round(pollIntervalMs / 1000)} 秒间隔反复查询，遇到 SUCCESS 或 FAILED 立即返回。返回值包含 pollCount、elapsedSeconds 和 timedOut，用于确认本次实际查询次数；RUNNING 且 timedOut=true 只表示本次等待窗口结束，不表示任务失败。复杂 DWG 图纸可能需要约 ${maxTaskDurationMinutes} 分钟，超时后请稍后再次调用 bangtu_wait_task 或 bangtu_get_task_status。`, {
        apiKey: apiKeySchema,
    ...
    server.tool("bangtu_get_frame_result", "获取 DWG 图纸基本信息识别的图框结果。仅当 PRE 任务状态为 SUCCESS 时调用。返回 data 图框数组，数组元素含 frameId、layoutName、frameWcsLoc、signInfo（图签识别内容）和 rotation。frameId 用于后续创建建筑构件识别任务。", {
        apiKey: apiKeySchema,
        taskId: z.string().min(1).describe("DWG 基本信息识别 PRE 任务的 taskId。"),
    }, async ({ apiKey, taskId }) => {
        const data = await callBangtuApi("/result/pre/frameBaseInfo", { query: { id: taskId }, apiKey });
        return textResult({ data, _hint: "已返回图框结果。选择需要识别的 frameId 后，可调用 bangtu_create_cv_task 创建建筑专业任务。" });
    });
    server.tool("bangtu_get_arch_result", "获取建筑构件识别结果。仅当 BUILDING_CV 任务状态为 SUCCESS 时调用。dataType 决定返回的结构化结果类型；接口路径、请求方法及 taskId 参数名均由本工具固化，调用者不需要也不能填写 endpoint。", {
        apiKey: apiKeySchema,
        ...
        const data = await callBangtuApi(`/result/building_cv/${dataType}`, { query: { id: taskId }, apiKey });
...
async function readLocalFile(filePath) {
    ...
    if (!filePath.toLowerCase().endsWith(".dwg"))
        throw new Error("filePath must point to a .dwg file");
    return { bytes, fileName: basename(filePath) };
}
async function downloadFile(fileUrl) {
    const response = await fetch(fileUrl);
    if (!response.ok)
        throw new Error(`Could not download DWG file: ${response.status} ${response.statusText}`);
    const url = new URL(fileUrl);
    const fileName = basename(url.pathname) || "drawing.dwg";
    if (!fileName.toLowerCase().endsWith(".dwg"))
        throw new Error("fileUrl must point to a .dwg file");
    return { bytes: new Uint8Array(await response.arrayBuffer()), fileName };
}
function decodeBase64File(fileBase64, fileName) {
    if (!fileName.toLowerCase().endsWith(".dwg"))
        throw new Error("fileName must end with .dwg");
    ...
    if (!bytes.length)
        throw new Error("fileBase64 is empty or invalid");
    return { bytes: new Uint8Array(bytes), fileName: basename(fileName) };
}
async function uploadDwg(file, apiKey) {
    const form = new FormData();
    const bytes = new Uint8Array(file.bytes);
    form.append("file", new Blob([bytes.buffer], { type: "application/octet-stream" }), file.fileName);
    return callBangtuApi("/pre/createPreTask", { method: "POST", formData: form, apiKey });
}
async function getTaskStatusWithHint(taskId, apiKey) {
    const data = await callBangtuApi("/result/taskStatusData", { query: { taskId }, apiKey });
    const intervalSeconds = Math.max(1, Math.round(pollIntervalMs / 1000));
    const hint = data.status === "SUCCESS"
        ? "任务已完成，可调用对应的结果获取工具。"
        : data.status === "FAILED"
...
async function callBangtuApi(path, input) {
    ...
    const headers = { Accept: "application/json", apiKey: input.apiKey };
    let body;
    if (input.json) {
    ...
    if (!isBangtuResponse(parsed))
        throw new Error(`BangTu API returned an invalid response: ${JSON.stringify(parsed)}`);
    if (parsed.code !== 200)
        throw new Error(`BangTu API business error ${parsed.code}: ${parsed.message}`);
    return parsed.data;
}
function isBangtuResponse(value) {
